...
function Builder({user , setUser}) {
  ...
  const testGeminiKey = async () => {
    ...
    try {
      const res = await axios.post(ServerUrl + "/api/user/test-key", { geminiApiKey: geminiApiKey.trim() }, { withCredentials: true })
      setKeyTestResult({ success: true, message: res.data.message })
      toast.success("API Key is valid!")
    } catch (error) {
      const msg = error.response?.data?.message || "Key validation failed"
      setKeyTestResult({ success: false, message: msg })
      toast.error(msg)
    }
    setTestingKey(false)
  }
  ...
  <script src="${CLIENT_URL}/assistant.js" data-user-id="${user?._id}"></script>
        ...
        {editAssistant && <div className='space-y-6'>
          ...
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
            <div className='flex items-center justify-between mb-5 gap-4 flex-wrap'>
              ...
              className='px-4 py-2 rounded-xl bg-linear-to-r from-purple-500 to-emerald-500 text-white text-sm font-medium hover:scale-[1.02] transition-all cursor-pointer'>
                Get API KEY
              </a>
            </div>
            ...
            <div className='flex gap-3'>
              ...
              className='px-5 py-3 rounded-2xl bg-blue-500 text-white text-sm font-medium whitespace-nowrap hover:bg-blue-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed'>
          ...
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
            <div className='flex items-center justify-between mb-5 flex-wrap'>
              ...
              <button onClick={addPage} className='flex items-center gap-2 px-4 py-2 rounded-xl bg-linear-to-r from-purple-500 to-emerald-500 text-white text-sm'>
                <FiPlus/>Add
              </button>
            </div>
        ...
        !businessType ||
        !businessDescription ||
        !geminiApiKey} className='w-full h-14 rounded-2xl bg-linear-to-r from-purple-500 to-emerald-500 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed'>
            {
